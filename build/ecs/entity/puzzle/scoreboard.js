...
class ComboCounter extends Entity {
    constructor(x, y) {
        super();
        this.addComponent(new Position(x, y));
        this.addComponent(new Score(ScoreType.COMBO));
        this.addComponent(new Drawable(ctx => {
            let position = this.getComponent(Position);
            let score = this.getComponent(Score).score.toString();
            let padding = " ".repeat(4 - score.length);
            score = padding + score;
            ctx.fillStyle = "#ddd";
            ctx.font = "30px serif";
            ctx.fillText("COMBO:", position.x, position.y);
            ctx.fillStyle = "white";
...
export { Scoreboard, ComboCounter };
